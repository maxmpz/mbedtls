...
void mbedtls_pk_free( mbedtls_pk_context *ctx )
{
    ...
    if ( ctx->pk_info != NULL ) {
    	DLOG("%s ctx=%p ctx->pk_info=%p", __FUNC__, ctx, ctx->pk_info);
    	DLOG("%s ctx->pk_info->ctx_free_func=%p", __FUNC__, ctx->pk_info->ctx_free_func);
...
int mbedtls_pk_setup( mbedtls_pk_context *ctx, const mbedtls_pk_info_t *info )
{
    ...
    DLOG("%s ctx->pk_info=>%p ctx->pk_info->ctx_free_func=%p", __FUNC__, ctx->pk_info, ctx->pk_info->ctx_free_func);
